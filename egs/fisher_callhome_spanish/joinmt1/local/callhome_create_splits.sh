...
for split in train devtest evltest; do
    ...
    awk 'BEGIN {FS=" "}; FNR==NR { a[$1]; next } ((substr($2,1,length($2)-2) ".sph") in a)' \
        $splitFile/$split $train_all/segments > $data_dir/$dirName/segments
    awk 'BEGIN {FS=" "}; FNR==NR { a[$1]; next } ((substr($1,1,length($1)-16) ".sph") in a)' \
        $splitFile/$split $train_all/text > $data_dir/$dirName/text

    n=$(awk 'BEGIN {FS = " "}; {print substr($2,1,length($2)-2)}' $data_dir/$dirName/segments | sort | uniq | wc -l)
