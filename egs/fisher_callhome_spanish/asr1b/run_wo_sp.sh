...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    dump.sh --cmd "$train_cmd" --nj 80 --do_delta $do_delta \
        data/${train_set}/feats.scp data/train_sp.es/cmvn.ark exp/dump_feats/${train_set} ${feat_tr_dir}
    # for x in ${train_dev} ${recog_set}; do
    #     feat_recog_dir=${dumpdir}/${x}/delta${do_delta}; mkdir -p ${feat_recog_dir}
    #     dump.sh --cmd "$train_cmd" --nj 32 --do_delta $do_delta \
    #         data/${x}/feats.scp data/train_sp.es/cmvn.ark exp/dump_feats/recog/${x} \
    #         ${feat_recog_dir}
    # done
fi

dict=data/lang_1spm/train_sp.es_${bpemode}${nbpe}_units_${src_case}.txt
nlsyms=data/lang_1spm/train_sp.es_non_lang_syms_${src_case}.txt
bpemodel=data/lang_1spm/train_sp.es_${bpemode}${nbpe}_${src_case}
...
lmexpname=train_sp.es_${src_case}_rnnlm_${backend}_${lmtag}_${bpemode}${nbpe}
