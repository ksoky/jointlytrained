...
dict=data/lang_${bpemode}/${train_set}_${bpemode}${nbpe}_units_${tgt_case}.txt
nlsyms=data/lang_${bpemode}/${train_set}_non_lang_syms_${tgt_case}.txt
bpemodel=data/lang_${bpemode}/${train_set}_${bpemode}${nbpe}_${tgt_case}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # #offset=$(wc -l < ${dict})
    # for lang in ${tgt_lang} en; do
    #     grep sp1.0 data/train_sp.en-${tgt_lang}.${lang}/text.${tgt_case} | cut -f 2- -d' ' | grep -v -e '^\s*$' > data/lang_${bpemode}/input.txt
...
# NOTE: can skip stage 3: LM Preparation
if [ -z ${lmtag} ]; then
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    grep sp1.0 data/${train_set}/text.${src_case} | cut -f 2- -d " " | spm_encode --model=${bpemodel}.en.model --output_format=piece \
        > ${lmdatadir}/train_${src_case}.txt
    cut -f 2- -d " " data/${train_dev}/text.${src_case} | spm_encode --model=${bpemodel}.en.model --output_format=piece \
