...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    # Divide into source and target languages dev.en-${tgt_lang} tst-COMMON.en-${tgt_lang} tst-HE.en-${tgt_lang}
    for x in ${train_set_prefix}.en-${tgt_lang}; do
        local/divide_lang.sh ${x} ${tgt_lang} ${case}
    done
    #dev.en-${tgt_lang}
    for x in ${train_set_prefix}.en-${tgt_lang}; do
...
dict=data/lang_1spm/train_sp.en-${tgt_lang}.en_${bpemode}${nbpe}_units_${case}.txt
nlsyms=data/lang_1spm/train_sp.en-${tgt_lang}.en_non_lang_syms_${case}.txt
bpemodel=data/lang_1spm/train_sp.en-${tgt_lang}.en_${bpemode}${nbpe}_${case}
