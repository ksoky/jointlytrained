...
D = r'([\u17b6-\u17d3\u17dd])'
M = r'([\u17d2])' # \u17d0 can't be middle because of writting error
#E = r'([\u17cb-\u17cd\u17cf])'
E = r'([\u17cb-\u17cf])'

def cluster(line):
    line = re.sub(r"([\u1780-\u17f9,.:])", r' \1 ', line, 0)
    line = " ".join([c for c in line.split() if c.strip()])
    line = re.sub(r" +"+D, r'\1', line, 0)
    line = re.sub(M+" +", r'\1', line, 0)
    line = re.sub(r" (.?)"+E, r' \1\2', line, 0)
    line = re.sub(r'([0-9]+) *([,.:]) *([0-9]+)', r'\1\2\3', line, 0)
    return line
