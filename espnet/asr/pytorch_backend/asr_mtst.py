...
class CustomConverter(object):
    """Custom batch converter for Pytorch.
        ...
        This will be use for joint ASR and MT
    Args:
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        #added to get mt source input
        # get batch of lengths of input sequences
        itlens = np.array([x.shape[0] for x in xt])

        # perform padding and convert to tensor
        xt_pad = pad_list([torch.from_numpy(x).long() for x in xt], 0).to(device)
        itlens = torch.from_numpy(itlens).to(device)


        return xs_pad, ilens, xt_pad, itlens, ys_pad

class CustomConverterAsrSt(object):
    """Custom batch converter for Pytorch.

        This will be use for joint ASR and ST
    Args:
        subsampling_factor (int): The subsampling factor.
        dtype (torch.dtype): Data type to convert.
        use_source_text (bool): use source transcription.
    ...
    """
    def __init__(self, subsampling_factor=1, dtype=torch.float32, use_source_text=False):
        """Construct a CustomConverter object."""
        self.subsampling_factor = subsampling_factor
        self.ignore_id = -1
        self.dtype = dtype
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        # get batch of lengths of input sequences of st
        itlens = np.array([x.shape[0] for x in xt])
        itlens = torch.from_numpy(itlens).to(device)

        xt_pad = pad_list([torch.from_numpy(x).float() for x in xt], 0).to(
            device, dtype=self.dtype
        )
        ...
        return xs_pad, ilens, xt_pad, itlens, ys_pad, ys_pad_src
...
def train(args):
    ...
    # if args.num_encs > 1:
    #     if args.model_module == "espnet.nets.pytorch_backend.e2e_asr_join_transformer:E2E":
    #         args = format_mulenc_args_transformer(args)
    #     else:
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        logging.info("input dim list {}.".format(idim_list))
    ...
    # Setup a converter
    if args.num_encs == 1:
        if args.join_train == "mt":
            converter = CustomConverter(
                subsampling_factor=model.subsample[0], 
                dtype=dtype
            )
        elif args.join_train == "st":
            converter = CustomConverterAsrSt(
                subsampling_factor=model.subsample[0],
                dtype=dtype,
...
def recog(args):
    ...
    load_inputs_and_targets = LoadInputsAndTargets(
        mode="asr",
        load_output=True if args.join_train == "mt" else False,
            ...
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    nbest_hyps = model.recognize(
                        feat, mt_feat, args, train_args.char_list, rnnlm
                    )
