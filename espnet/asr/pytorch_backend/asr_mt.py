...
#from espnet.asr.asr_utils import add_results_to_json #this is the correct one, I will update my json data later
from espnet.asr.asr_utils import add_results_to_json_join
from espnet.asr.asr_utils import CompareValueTrigger
from espnet.asr.asr_utils import format_mulenc_args, format_mulenc_args_transformer
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        # perform padding and convert to tensor
        xt_pad = pad_list([torch.from_numpy(x).long() for x in xt], self.pad).to(device)
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                new_js[name] = add_results_to_json_join(
