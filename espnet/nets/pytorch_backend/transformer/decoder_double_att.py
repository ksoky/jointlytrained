...
from espnet.nets.pytorch_backend.transformer.decoder_layer_double_att import DecoderLayer
...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask , memory, memory_mask, mt_memory, mt_memory_mask):
        """Forward decoder.
        ...
        x, tgt_mask, memory, memory_mask, mt_memory, mt_memory_mask = self.decoders(
            x, tgt_mask, memory, memory_mask, mt_memory, mt_memory_mask
        )
    ...
    def forward_one_step(self, tgt, tgt_mask, memory, mt_memory, cache=None):
        """Forward one step.
        ...
        for c, decoder in zip(cache, self.decoders):
            x, tgt_mask, memory, memory_mask, mt_memory, mt_memory_mask = decoder(
                x, tgt_mask, memory, None, mt_memory, None, cache=c
            )
            new_cache.append(x)
    ...
    def score(self, ys, state, x, xt):
        ...
        logp, state = self.forward_one_step(
            ys.unsqueeze(0), ys_mask, x.unsqueeze(0), xt.unsqueeze(0), cache=state
        )
        return logp.squeeze(0), state

    # batch beam search API (see BatchScorerInterface)
    def batch_score(
        self, ys: torch.Tensor, states: List[Any], xs: torch.Tensor, xt: torch.Tensor
        ...
        logp, states = self.forward_one_step(ys, ys_mask, xs, xt, cache=batch_state)
